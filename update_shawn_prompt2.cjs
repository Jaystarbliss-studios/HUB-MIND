...
code = code.replace(/const contextPrompt = \`You are Shawn, an administrative AI assistant embedded within the Hub-Mind platform\.[\s\S]*?without user confirmation\.\`;/m, 'const contextPrompt = `' + newPrompt.replace(/\`/g, "\\\`") + '`;');
