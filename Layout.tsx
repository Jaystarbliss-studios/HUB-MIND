...
import { LayoutDashboard, CheckSquare, Users, Calendar, Folder, Bell, LogOut, Settings, Inbox, Plus, X, Brain, Book, Briefcase, Search, Clock3, FileText } from 'lucide-react';
...
export function Layout() {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden print:h-auto print:overflow-visible print:bg-white">
      ...
      {/* Mobile Quick Action Choice Sheet (Quick Thought vs Create Document) */}
      {showQuickActionSheet && (
        <div
          className="md:hidden fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-end"
          onClick={() => setShowQuickActionSheet(false)}
        >
          <div
            className="w-full bg-slate-900 border-t border-slate-800 rounded-t-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-200 pb-[calc(1rem+env(safe-area-inset-bottom))]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-center p-4 border-b border-slate-800">
              <h3 className="font-bold text-white">Quick Action</h3>
              <button onClick={() => setShowQuickActionSheet(false)} className="text-slate-500 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-3 flex flex-col gap-2">
              <button
                ...
                className="flex items-center gap-3 w-full p-3.5 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-800 text-left transition-colors"
              >
                <div className="w-9 h-9 rounded-lg bg-accent/15 text-accent flex items-center justify-center shrink-0">
                  <Inbox className="w-4.5 h-4.5" />
                </div>
                <div>
                  <div className="font-semibold text-sm text-white">Quick Thought</div>
                  <div className="text-xs text-slate-400">Drop a note in your Inbox</div>
                </div>
              </button>
              {(profile?.role === 'admin' || profile?.role === 'assistant') && (
                ...
                >
                  ...
                  <div>
                    <div className="font-semibold text-sm text-white">Create Document</div>
                    <div className="text-xs text-slate-400">Pick a template and start writing</div>
                  </div>
                </button>
              )}
            </div>
