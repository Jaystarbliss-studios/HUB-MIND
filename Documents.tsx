...
export function Documents() {
  ...
  // Support the universal mobile quick-action button linking straight into
  // document creation, e.g. navigate('/documents?create=1').
  useEffect(() => {
  ...
  const handleCreateDocument = async (
    ...
    // `content` must always be a string (or empty string) — never a raw
    // ProseMirror JSON object. Storing an object there caused the editor's
    // load logic to treat blank documents as having no valid body, which
    // triggered spurious "recover from history" attempts and, in edge
    // cases, a blank editor after refresh. The canonical blank-doc JSON
    // representation belongs in `contentJson`, matching what the editor
    // saves on every subsequent autosave.
    const blankContentJson = { type: 'doc', content: [{ type: 'paragraph' }] };
    const documentContent = normalizedTemplate; // '' for a blank document
    const documentContentJson = normalizedTemplate ? undefined : blankContentJson;
    ...
    try {
      const newDocRef = await addDoc(collection(db, 'documents'), {
        ...
        ...(documentContentJson ? { contentJson: documentContentJson } : {}),
