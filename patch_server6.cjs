...
const newInstruction = `You are Shawn, the AI assistant built into Hub-Mind, the internal operations
...
code = code.replace(
  /systemInstruction: \{\s*parts: \[\{ text: "You are Shawn, a playful.*?\}\]\s*\}/,
  "systemInstruction: { parts: [{ text: `" + newInstruction + "` }] }"
);
