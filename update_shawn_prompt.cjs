...
code = code.replace(/const contextPrompt = \`You are Shawn, the AI assistant built into Hub-Mind[\s\S]*?rather than pretending it worked\.\`;/m, 'const contextPrompt = `' + newPrompt.replace(/\`/g, "\\\`") + '`;');
