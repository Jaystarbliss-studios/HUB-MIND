...
export function getLocalTasks(): Task[] { return readArray<Task>(TASKS_KEY); }
...
export function deleteLocalTask(taskId: string) {
  setLocalTasks(getLocalTasks().filter(t => t.id !== taskId));
}

export function getLocalProjects(): Project[] { return readArray<Project>(PROJECTS_KEY); }
...
export function deleteLocalProject(projectId: string) {
  setLocalProjects(getLocalProjects().filter(p => p.id !== projectId));
}

export function getLocalClients(): Client[] { return readArray<Client>(CLIENTS_KEY); }
...
export function deleteLocalClient(clientId: string) {
  setLocalClients(getLocalClients().filter(c => c.id !== clientId));
}

export function getLocalMeetings(): Meeting[] { return readArray<Meeting>(MEETINGS_KEY); }
...
export function deleteLocalMeeting(meetingId: string) {
  setLocalMeetings(getLocalMeetings().filter(m => m.id !== meetingId));
}

export function getLocalFollowUps(): FollowUp[] { return readArray<FollowUp>(FOLLOWUPS_KEY); }
...
export function deleteLocalFollowUp(followUpId: string) {
  setLocalFollowUps(getLocalFollowUps().filter(f => f.id !== followUpId));
}
