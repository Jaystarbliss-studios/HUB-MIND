...
export async function saveDocumentOffline(
  ...
  const updatedRecord: OfflineDocRecord = {
    ...
    // Only ever set true once the Firestore write below actually succeeds.
    // Marking this true just because the browser reports itself online was
    // optimistic and wrong: if the page unloads (refresh/navigation) before
    // the awaited updateDoc resolves, the write can be cut off mid-flight,
    // leaving Firestore stale while local storage incorrectly claims to be
    // synced. That false "synced: true" then stops later loads from
    // trusting the newer local copy over the stale cloud copy.
    synced: false,
  };
